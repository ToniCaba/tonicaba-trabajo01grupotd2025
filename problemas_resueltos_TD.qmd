...
Observamos que para un valor de alpha en el intervalo (0.2, 0.4), la alternativa óptima pasa de ser la 2 (Correr) a la 3 (Montar en bici). Veamos con el siguiente gráfico en que punto ocurre esto.
...
El punto en el que la alternativa óptima pasa de ser la segunda (Correr) a la tercera (Montar en bici) es 0.25.
...
```{r}
# Matriz de beneficios por proveedor (miles de euros)
energia <- matrix(
  c(60, 80, 100,
    50, 100, 120,
    70, 90, 110),
...
decision_incertidumbre <- function(tabla, alpha = 0.6) {
  ...
  regret <- sweep(tabla, 2, mejores, "-")  # <- CORREGIDO
